...
function UserManagement() {
      ...
      processedData.forEach(user => {
        ...
        user.group_name.forEach(group => {
          options.push({
            value: group,
            label: group,
            isFixed: group === "Admin" && (appState.username === user.username || user.username === "admin0")
          })
        })
  ...
  async function handleCreateGroup() {
    if (c_groupname !== "") {
      try {
        ...
        const response = await Axios.post("/group/create", { new_group })
        if (response.data) {
          toast("New group is created.")
          console.log(response.data)
          setc_groupname("")
          fetchUsers()
        }
      } catch (e) {
        toast("Group is not created. Check for duplicate group name.")
        console.log(e)
        console.log("There was a problem.")
      }
    }
  ...
  async function handleCreateUser() {
    ...
    if (c_username !== "" && validatePassword(c_password) && email_checker) {
      ...
      toast("Please check input fields again.")
  ...
  async function handleUpdatePassword(u_password, u_username) {
    if (validatePassword(u_password)) {
      try {
        ...
        if (response.data) {
          toast(`password is updated for ${u_username}`)
          console.log(response.data)
        } else {
  ...
  async function handleUpdateEmail(u_email, u_username) {
    try {
      ...
      if (response.data) {
        toast(`email is updated for ${u_username}`)
        console.log(response.data)
      } else {
  ...
  async function handleUpdateActive(u_activestatus, u_username) {
    try {
      ...
      if (response.data) {
        toast(`status is updated to ${u_activestatus} for ${u_username}`)
      } else {
        console.log(`backend did not response well`)
        console.log(response.data)
      }
  ...
  async function handleGroupSelect(u_group, u_username) {
    try {
      ...
      if (response.data) {
        console.log(response.data)
        toast(`${u_username} has been added to ${u_group}`)
      } else {
        console.log(response.data)
  ...
  async function handleGroupRemove(u_group, u_username) {
    try {
      ...
      if (response.data) {
        console.log(response.data)
        toast(`${u_username} has been removed from ${u_group}`)
      } else {
        console.log(response.data)
        console.log("something is wrong")
      }
                      ...
                      <input
                        onBlur={e => {
                          if (e.target.value !== user.email && validator.isEmail(e.target.value)) {
                            ...
                            toast("Email not updated. Please check input again.")
                          }
                        }}
                        type="email"
                        className="form-control"
