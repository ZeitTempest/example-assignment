...
async function sendEmail(req, res, next) {
  ...
  connectDatabase.getConnection(function (err, connection) {
    ...
    const query1 = "SELECT username FROM tms_database.user_groups WHERE group_name = 'ProjectLead';"
...
exports.PromoteTask2Done = async (req, res, next) => {
  ...
  connectDatabase.getConnection(async function (err, connection) {
    ...
    const appPermissionDoing = await new Promise((resolve, reject) => {
      const query2 = "SELECT App_permit_Doing FROM application WHERE App_Acronym = ?"
